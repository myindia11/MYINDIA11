...
public class CVCBowlAdapter extends RecyclerView.Adapter<CVCBowlAdapter.ViewHolder> {
    ...
    public CVCBowlAdapter(Context context, List<BowlerModel> bowlerModelList) {
        this.context = context;
        this.bowlerModelList = bowlerModelList;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        BowlerModel playersListModel = bowlerModelList.get(position);
                ...
                if (snapshot.exists()) {
                    ...
                    if (playersListModel.getPlayerName().equals(scap)) {
                        ...
                        databaseReference3.child("FinalCreatedTeamsPlayer").child(uid).child(Common.avlContestId).child(Common.teamCode)
                                .child("Bowler").child(playersListModel.getPid()).child("cap").setValue("NO");
                    }
                    if (playersListModel.getPlayerName().equals(svcap)) {
                        ...
                        databaseReference3.child("FinalCreatedTeamsPlayer").child(uid).child(Common.avlContestId).child(Common.teamCode)
                                .child("Bowler").child(playersListModel.getPid()).child("vcap").setValue("NO");
                    }
                }
        ...
        holder.identification.setImageDrawable(ContextCompat.getDrawable(context,R.drawable.bowler_icon));
        ...
        holder.capBtn.setOnClickListener(view -> {
            ...
            captain = bowlerModelList.get(position).getPlayerName();
            viceCaptain = svcap;
            ...
            teamName = bowlerModelList.get(position).getTeamName();
            playerName = bowlerModelList.get(position).getPlayerName();
            pid = bowlerModelList.get(position).getPid();
            playerStatus = bowlerModelList.get(position).getPlayerStatus();
            playerRole = "Bowler";
            isSelected = true;
            creditScore = bowlerModelList.get(position).getCreditScore();
            obtainedPoints = bowlerModelList.get(position).getObtainedPoints();
            cap = "YES";
            vcap = "NO";
            playerPic = bowlerModelList.get(position).getPlayerPic();
        ...
        holder.vcapBtn.setOnClickListener(view -> {
            ...
            teamTwo = Common.teamTwoName;
            captain = scap;
            viceCaptain = bowlerModelList.get(position).getPlayerName();
            teamOneCount = tmOneCount;
            teamTwoCount = tmTwoCount;
            PlayerValuesModel playerValuesModel = new PlayerValuesModel(teamCode,leftCredit,WK,BT,AR,BR,teamOneCount,teamTwoCount,
                    teamOne, teamTwo, captain, viceCaptain);
