...
public class CVCWiketAdapter extends RecyclerView.Adapter<CVCWiketAdapter.ViewHolder> {
    public Context context;
    private List<WicketModel> wicketModelList;
    ...
    public CVCWiketAdapter(Context context, List<WicketModel> wicketModelList) {
        this.context = context;
        this.wicketModelList = wicketModelList;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        WicketModel playersListModel = wicketModelList.get(position);
                ...
                if (snapshot.exists()) {
                    ...
                    if (playersListModel.getPlayerName().equals(scap)) {
                        ...
                        databaseReference3.child("FinalCreatedTeamsPlayer").child(uid).child(Common.avlContestId).child(Common.teamCode)
                                .child("WicketKeeper").child(playersListModel.getPid()).child("cap").setValue("NO");
                    }
                    if (playersListModel.getPlayerName().equals(svcap)) {
                        ...
                        databaseReference3.child("FinalCreatedTeamsPlayer").child(uid).child(Common.avlContestId).child(Common.teamCode)
                                .child("WicketKeeper").child(playersListModel.getPid()).child("vcap").setValue("NO");
                    }
                }
            }
        ...
        holder.capBtn.setOnClickListener(view -> {
            ...
            captain = wicketModelList.get(position).getPlayerName();
            viceCaptain = svcap;
            ...
            teamName = wicketModelList.get(position).getTeamName();
            playerName = wicketModelList.get(position).getPlayerName();
            pid = wicketModelList.get(position).getPid();
            playerStatus = wicketModelList.get(position).getPlayerStatus();
            playerRole = "WicketKeeper";
            isSelected = true;
            creditScore = wicketModelList.get(position).getCreditScore();
            obtainedPoints = wicketModelList.get(position).getObtainedPoints();
            cap = "YES";
            vcap = "NO";
            playerPic = wicketModelList.get(position).getPlayerPic();
        ...
        holder.vcapBtn.setOnClickListener(view -> {
            ...
            teamTwo = Common.teamTwoName;
            captain = scap;
            viceCaptain = wicketModelList.get(position).getPlayerName();
            teamOneCount = tmOneCount;
            teamTwoCount = tmTwoCount;
            PlayerValuesModel playerValuesModel = new PlayerValuesModel(teamCode,leftCredit,WK,BT,AR,BR,teamOneCount,teamTwoCount,
                    teamOne, teamTwo, captain, viceCaptain);
